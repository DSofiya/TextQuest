...
def load_buttons(WINDOW_SIZE):
    buttons_data = [
        {"x": WINDOW_SIZE[0] / 1.43, "y": WINDOW_SIZE[1] / 6.6, "width": WINDOW_SIZE[0] / 20,
            "height": WINDOW_SIZE[0] / 20, "image_path": "Menu_images\\arroy2_up.png", "text": "", },
        {"x": WINDOW_SIZE[0] / 1.43, "y": WINDOW_SIZE[1] / 3, "width": WINDOW_SIZE[0] / 20,
            "height": WINDOW_SIZE[0] / 20, "image_path": "Menu_images\\arroy2_down.png", "text": "", },
        {"x": WINDOW_SIZE[0] / 1.30, "y": WINDOW_SIZE[1] / 8, "width": WINDOW_SIZE[0] / 11,
            "height": WINDOW_SIZE[1] / 5.5, "image_path": "Character\\exit.png", "text": "Вихід до головного меню ", }, ]

    buttons = [
        Button(data["x"], data["y"], data["width"], data["height"], data["image_path"], data["text"], )
...
def find_text(prompt_key):
    with open('Text_patern\\Rosefild.json', 'r', encoding='utf-8') as файл:
        text_list = json.load(файл)
...
# def get_prompt_text():
    ...
    if prompt_text == "":  
        list = ["хазяїн","лікар","доктор","розкажи","поговори","підійти",
                "настоят","храм","магаз","алхімік","власник","зіл","заклин","невидимост",
                "світіння","катакомб","ввійти", "війти","зайти","повернутись","далі","так","є","карт","закляття",
                "стати","напаст","підійти","убити","наступного","втікти","подарувати","продати","вкрасти",
                "розбити","битись","прислужн","else"] 
...
def refresh_window (buttons,button_font,WINDOW_SIZE,window,bg_image):
    image_fon = image("Menu_images\\book.png",(WINDOW_SIZE[0] / 2, WINDOW_SIZE[0] / 2.5))
    image_ansver = image("Menu_images\\ansver.png",(WINDOW_SIZE[0] / 1.6, WINDOW_SIZE[0] /12))
...
def render_chat_response(chat_response, menu_font, scroll_y,window,WINDOW_SIZE):
    ...
    for line in displayed_lines:
        ...
        if y_position + scroll_y >= WINDOW_SIZE[1] // 5 and y_position + scroll_y <= WINDOW_SIZE[1] // 1.6:
...
def main():
    ...
    if response is not None :
        ...
        chat_response = grate_into_rows(response.choices[0].text.strip(),menu_font,WINDOW_SIZE)
                ...
                for button in buttons:
                    if button.is_clicked(pos):
                        if button == buttons[-1]:
                            ...
                            scroll_y += 20
                        elif button == buttons[1] and scroll_y > -(len(chat_response) * menu_font.get_linesize() - WINDOW_SIZE[1] // 5):
                            scroll_y -= 20
